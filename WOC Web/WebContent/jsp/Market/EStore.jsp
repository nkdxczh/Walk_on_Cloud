...
function sort(type){
	...
	window.location="/WOC/SearchCommoditiesListServlet?key=${requestScope.key}&pageNumber=1&jsp=3&sort="+type;
}
function order(Id){
...
<body  onload="onPageLoad();dwr.engine.setActiveReverseAjax(true);dwr.engine.setNotifyServerOnPageUnload(true);;">
	...
	<c:if test="${empty requestScope.flag }">
		<script>window.location="/WOC/SearchCommoditiesListServlet?key=11&pageNumber=1&sort=1&jsp=3"; </script>
	</c:if>
		...
		<div class="section_container">
			...
			<div id="side_nav">
				<div class="sideNavCategories">
					<h1 style="font-family: 宋体; font-size: 20px;">交换商品</h1>
					<ul class="category departments">
						<li class="header" style="font-family: 宋体; font-size: 16px;">分类</li>
						<li><a
							href="/WOC/SearchCommoditiesListServlet?key=4&pageNumber=1&sort=1&jsp=3">书籍</a></li>
						<li><a
							href="/WOC/SearchCommoditiesListServlet?key=5&pageNumber=1&sort=1&jsp=3">日用品</a></li>
						<li><a
							href="/WOC/SearchCommoditiesListServlet?key=6&pageNumber=1&sort=1&jsp=3">其他</a></li>
					</ul>
					<ul class="category departments">
						<li class="header" style="font-family: 宋体; font-size: 16px;">关注度</li>
						<li><a
							href="/WOC/SearchCommoditiesListServlet?key=16&pageNumber=1&sort=1&jsp=3">20以下</a></li>
						<li><a
							href="/WOC/SearchCommoditiesListServlet?key=17&pageNumber=1&sort=1&jsp=3">20到100</a></li>
						<li><a
							href="/WOC/SearchCommoditiesListServlet?key=18&pageNumber=1&sort=1&jsp=3">100以上</a></li>
					</ul>
					<ul class="category departments">
						<li class="header" style="font-family: 宋体; font-size: 16px;">发布时间</li>
						<li><a
							href="/WOC/SearchCommoditiesListServlet?key=25&pageNumber=1&sort=1&jsp=3">一周内</a></li>
						<li><a
							href="/WOC/SearchCommoditiesListServlet?key=26&pageNumber=1&sort=1&jsp=3">一个月内</a></li>
						<li><a
							href="/WOC/SearchCommoditiesListServlet?key=27&pageNumber=1&sort=1&jsp=3">一个月以上</a></li>
					</ul>
				</div>
			</div>
			...
			<div id="main_content">
				...
				<ul class="breadcrumb">
					...
					<li class="active"><a href="jsp/Market/EStore.jsp">交换商城</a></li>
				</ul>
				<!--Toolbar-->
				<div class="toolbar">
					<div class="sortby">
						...
						<c:forEach items="${requestScope.lstCommodities }" var="com">
							<li><a class="product_image"><img
								...
								<div class="price_info">
									<a href="/WOC/ApplyAttenttionSevlet?comId=${com.comId }&jsp=3">+
										加关注</a>
				...
				<div class="page" style="margin-bottom: 10px; margin-left: 250px;">
					<a
						href="/WOC/SearchCommoditiesListServlet?key=${requestScope.key }&pageNumber=${requestScope.pageNumber-1 }&sort=${requestScope.sort }&jsp=3">上一页</a>
					<c:forEach var="i" begin="1" end="${requestScope.pageCount }">
						<c:if test="${requestScope.pageNumber==i }">
							<a style="font-size: 20px;"
								href="/WOC/SearchCommoditiesListServlet?key=${requestScope.key }&pageNumber=${i }&sort=${requestScope.sort }&jsp=3">${i }</a>
						</c:if>
						<c:if test="${requestScope.pageNumber!=i }">
							<a style="width: 10px"
								href="/WOC/SearchCommoditiesListServlet?key=${requestScope.key }&pageNumber=${i }&sort=${requestScope.sort }&jsp=3">${i }</a>
					...
					<a
						href="/WOC/SearchCommoditiesListServlet?key=${requestScope.key }&pageNumber=${requestScope.pageNumber+1 }&sort=${requestScope.sort }&jsp=3">下一页</a>
				</div>
				<div class="toolbar">
					<div class="sortby">
						<label>排序:</label> <select onchange="sort(this.value)">
