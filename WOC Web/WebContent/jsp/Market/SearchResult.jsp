...
function sort(type){
	...
	window.location="/WOC/SearchCommoditiesServlet?key="+key+"&sort=1&option="+option+"&pageNumber="+pageNumber;
}
function order(Id){
...
<body  onload="onPageLoad();dwr.engine.setActiveReverseAjax(true);dwr.engine.setNotifyServerOnPageUnload(true);;">
    ...
    if(request.getAttribute("flag")==null){
        ...
        response.sendRedirect("/WOC/SearchCommoditiesServlet?key="+key+"&sort=1&option="+option+"&pageNumber="+index);
    }
%>
...
<div id="123" style="width:220px; height:100px; z-index:9998; position:fixed; left:70%; top:10%; background-color : #F0F7F0; border:solid 2px #F0E0FE; opacity:0.7 ; text-align:center; display: none" >
	...
	<div class="wrapper">
		...
		<div class="navigation_container">
			...
			<div class="minicart">
				<a href="#" class="minicart_link"> <span class="item"><b>我的订单</b></span>
		...
		<div class="section_container">
			...
			<div id="main_content" style="width: 100%">
				...
				<!--Product List Ends-->
				<!--Toolbar-->
				<div class="page" style="margin-bottom: 10px; margin-left: 500px;">
					<a href="" /WOC/SearchCommoditiesServlet?key=${requestScope.key }
						&sort=1&option=${requestScope.option
						}&pageNumber=${requestScope.pageNumber-1 }">上一页</a>
					<c:forEach var="i" begin="1" end="${requestScope.pageCount }">
						<c:if test="${requestScope.pageNumber==i }">
							<a style="font-size: 20px;"
								href="/WOC/SearchCommoditiesServlet?key=${requestScope.key }&sort=1&option=${requestScope.option }&pageNumber=${i }">${i }</a>
						</c:if>
						<c:if test="${requestScope.pageNumber!=i }">
							<a style="width: 10px"
								href="/WOC/SearchCommoditiesServlet?key=${requestScope.key }&sort=1&option=${requestScope.option }&pageNumber=${i }">${i }</a>
					...
					<a
						href="/WOC/SearchCommoditiesServlet?key=${requestScope.key }&sort=1&option=${requestScope.option }&pageNumber=${requestScope.pageNumber+1 }">下一页</a>
				</div>
				<div class="toolbar">
					<div class="sortby">
						<label>排序:</label> <select onchange="sort(this.value)">
