...
<body  onload="onPageLoad();dwr.engine.setActiveReverseAjax(true);dwr.engine.setNotifyServerOnPageUnload(true);;">
	<c:if test="${empty requestScope.flag }">
		<script>
			window.location = "/WOC/GetLatestPostsByLayoutServlet?posttype=300";
		</script>
	</c:if>
...
<div><img src="jsp/forum/images/music_bg.jpg" alt="" width="1348" height="484"/></div>
...
<div class="content">
	<h2>最新帖子</h2>
				<div class="post">
					 <h3 style="width:50%">标题</h3></a>
                     <h4 style="width:20%">发帖人</h4>
                     <h5 style="width:30%">发帖时间</h5>
			   </div>
			   <c:forEach items="${requestScope.lstLatestPostByLayout }" var="post">
				<div class="post" style="overflow:hidden" >
					 <a href="jsp/forum/post.jsp?postid=${post.postId }"><h3 style="width:50%">${post.postName } </h3></a>
                     <h4 style="width:20%">${post.userName }</h4>
                     <h5 style="width:30%">${post.postTime }</h5>
			   </div>
			   </c:forEach>
</div>
<div class="sidebar">
