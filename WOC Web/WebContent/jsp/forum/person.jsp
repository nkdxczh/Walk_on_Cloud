...
function talk(fid){
	...
	if(user=="")alert("请先登录！");
		...
		window.location="/WOC/TalkServlet?aimId="+fid;
	}
}
function release(){
	var user="${sessionScope.user.userId}";
...
<body
	...
	<c:if test="${!empty requestScope.tip }">
		<c:if test="${requestScope.tip=='1' }">
			<script>window.alert("添加好友成功！");</script>
		</c:if>
		<c:if test="${requestScope.tip=='2' }">
			<script>window.alert("添加好友失败！");</script>
		</c:if>
	</c:if>
	<%  
    if(request.getAttribute("flag")==null){
        int p=Integer.parseInt(request.getParameter("userid"));
        response.sendRedirect("/WOC/ShowOthersInfServlet?userid="+p);
    }
%>
		...
		<div class="section_container">
			...
			<div id="product_detail">
				...
				<div class="product_leftcol">
					<img src="/WOC/jsp/user_images/${requestScope.userinf.userPhoto }" />
					<span class="pr_info"></span>
				</div>
				<!--Product Left Ends-->
				<!--Product Right Starts-->
				<div class="product_rightcol">
					<small class="pr_type">用户信息</small>
					<h1 style="font-size: 30px;">${requestScope.userinf.nickName }</h1>
					<div class="size_info">
						<div class="size_sel">
							<label style="font-size: 15px;">性别
								：${requestScope.userinf.gender }</label>
						</div>
						<div class="color_sel">
							<label style="font-size: 15px;">积分
								：${requestScope.userinf.score}</label>
						</div>
					</div>
					<div class="size_info">
						<div class="size_sel">
							<label style="font-size: 15px;">专业
								：${requestScope.userinf.major }</label>
						</div>
						<div class="color_sel">
							<label style="font-size: 15px;">住址
								：${requestScope.userinf.location}</label>
						</div>
					</div>
					<div class="size_info">
						<div class="size_sel">
							<label style="font-size: 15px;">电话
								：${requestScope.userinf.phone }</label>
						</div>
						<div class="color_sel">
							<label style="font-size: 15px;">邮编
								：${requestScope.userinf.postCode}</label>
						</div>
					</div>
					<div class="size_info">
						<div class="size_sel">
							<label style="font-size: 15px;">Email
								：${requestScope.userinf.email }</label>
						</div>
						<div class="color_sel">
							<label style="font-size: 15px;">爱好
								：${requestScope.userinf.hobby}</label>
						</div>
					</div>
					<div class="add_to_buttons">
						<button onclick="addfriend('${requestScope.userinf.userId}');"
							class="add_cart">加为好友</button>
						<button class="add_cart"
							onclick="talk('${requestScope.userinf.userId}');">发起对话</button>
					</div>
					<div class="product_overview" style="height: 100px">
						<h4>ta发过的帖子</h4>
						<marquee vspace="20" behavior="scroll" scrollAmount="1"
							scrollDelay="0" direction="up" width="98%" height="80px"
							valign="top" onMouseOut="this.start()" onMouseOver="this.stop()">
							<ul>
								<c:forEach items="${requestScope.lstpost }" var="post">
									<li><a href="jsp/forum/post.jsp?postid=${post.postId }">${post.postName }</a></li>
								</c:forEach>
							</ul>
						</marquee>
					</div>
