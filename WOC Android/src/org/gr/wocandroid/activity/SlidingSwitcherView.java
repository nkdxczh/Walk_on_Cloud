...
public class SlidingSwitcherView extends RelativeLayout implements
	...
	/**
	 * * 重写SlidingSwitcherView的构造函数，用于允许在XML中引用当前的自定义布局。 * * @param context * @param
	 * attrs
	 */
	...
	/**
	 * * 当前是否能够滚动，滚动到第一个或最后一个元素时将不能再滚动。 * * @return
	 * 当前leftMargin的值在leftEdge和rightEdge之间返回true,否则返回false。
	 */
	...
	/**
	 * * 判断当前手势的意图是不是想滚动到上一个菜单元素。如果手指移动的距离是正数，则认为当前手势是想要滚动到上一个菜单元素。 * * @return
	 * 当前手势想滚动到上一个菜单元素返回true，否则返回false。
	 */
	...
	/**
	 * * 判断当前手势的意图是不是想滚动到下一个菜单元素。如果手指移动的距离是负数，则认为当前手势是想要滚动到下一个菜单元素。 * * @return
	 * 当前手势想滚动到下一个菜单元素返回true，否则返回false。
	 */
	 ...
	 * 就认为应该滚动到下一个菜单元素。 * * @return 如果应该滚动到下一个菜单元素返回true，否则返回false。
	 */
	 ...
	 * 就认为应该滚动到上一个菜单元素。 * * @return 如果应该滚动到上一个菜单元素返回true，否则返回false。
	 */
	...
	/** * 获取手指在右侧布局的监听View上的滑动速度。 * * @return 滑动速度，以每秒钟移动了多少像素值为单位。 */
	...
	/**
	 * * 检测菜单滚动时，是否有穿越border，border的值都存储在{@link #borders}中。 * * @param
	 * leftMargin * 第一个元素的左偏移值 * @param speed * 滚动的速度，正数说明向右滚动，负数说明向左滚动。 * @return
	...
	/**
	 * * 找到离当前的leftMargin最近的一个border值。 * * @param leftMargin * 第一个元素的左偏移值 * @return
	 * 离当前的leftMargin最近的一个border值。
	 */
	...
	/** * 使当前线程睡眠指定的毫秒数。 * * @param millis * 指定当前线程睡眠多久，以毫秒为单位 */
