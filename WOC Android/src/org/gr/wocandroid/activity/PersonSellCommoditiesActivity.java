...
public class PersonSellCommoditiesActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if(lstData.size()==0)
		{
			intent.setClass(PersonSellCommoditiesActivity.this, InformationActivity.class);
			startActivity(intent);
			Toast.makeText(getApplicationContext(), "您的交易商品信息为空", Toast.LENGTH_SHORT).show();
		}
	...
	private List<Map<String, ?>> fetchData() {
		...
		String response = WebAccessUtils.httpRequest("AShowUserSellCommoditiesServlet",
				lstNameValuePairs).trim();
        System.out.println(response+"DDDDDDDDDDDDDDDDDDDDDDDDDDDDDD");
		// 步骤4-3：设置一个全新的类型Type
		Type ListInformation = new TypeToken<ArrayList<Commodities>>() {
		}.getType();
		// 步骤4-4：创建并实例化一个Gson对象

		// 步骤4-5：解析JSon数据
		List<Commodities> lstInformation = gson.fromJson(response, ListInformation);
 
		
		// 步骤4-6：使用循环遍历集合对象
		for (Commodities commodity : lstInformation) {
			Map<String, Object> item = new HashMap<String, Object>();
			item.put("commodity_name", commodity.getComName());
			item.put("commodity_price", commodity.getPrice());
			item.put("commodity_describe", commodity.getDescribe());
			item.put("commodity_owner_name",  commodity.getOwnerName());
			item.put("commodity_location", commodity.getComRegion());
